    ...
    if st.session_state.selected_model == "one_feature":
        ...
        def data_loading(ticker):
            ...
            if not recent_d.empty:
                ...
                predict = predict_future_values(model, recent_d_value, scaler, 90, 10)
        ...
        data = yf.download("NHPC.NS", start=start_date, end=end_date,auto_adjust=False)
        ...
        recent_data = data.tail(100).reset_index()

        features = ['Open', 'High', 'Low', 'Close', 'Volume', '50sma', '200sma', '14rsi']

        if not (recent_data is None or recent_data.empty):
            scaled_data = scaler.fit_transform(recent_data[features])
            scaled_data = scaled_data.reshape(1,100,8)
